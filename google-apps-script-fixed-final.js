// Google Apps Script untuk mengelola Users, Products, dan Orders - DIPERBAIKI SELLER UPDATE
 ...
 * @return {string|null} Email seller atau null jika tidak ditemukan
 */
function getSellerEmailByProductId(productId) {
  ...
  const product = data.slice(1).find(row => row[0] === productId); // product_id di kolom 0
  return product ? product[1] : null; // user_id (email) di kolom 1
}
...
/**
 * Menangani CRUD untuk sheet Orders - DIPERBAIKI LOGIC SELLER
 * @param {Object} json - Data JSON dari permintaan
 * @return {GoogleAppsScript.Content.TextOutput} Respons
 */
  ...
  try {
    ...
    switch (action) {
      case "create":
        ...
        // PERBAIKAN: Dapatkan seller email berdasarkan product_id
        ...
        const row = [
          orderId, 
          userId, 
          sellerEmail, // GUNAKAN EMAIL SELLER, BUKAN UUID
          data.product_id || "", 
          data.quantity || 1, 
          data.total_price || 0, 
      ...
      case "update":
        ...
        if (updateIndex !== -1) {
          ...
          // PERBAIKAN: Bandingkan email dengan email seller yang tersimpan
